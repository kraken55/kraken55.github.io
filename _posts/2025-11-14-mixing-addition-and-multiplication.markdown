...
This is very important: **the order of operations is not a mathematical rule**. It's a convention to help us
...
Technically, I can perform the addition inside the parentheses and proceed as before, but let's instead break up the multiplication
...
There is no magic involved. Nothing special is happening. I simply chose to *rewrite* multiplication as repeated addition.
This does not violate the order of operations, since I haven't performed any operation yet 😉.
...
The order of operations plays an important role only when multiple different types of operations are involved.
With addition only, you are free to jumble the terms around.

I can reorder the above expression like so:
\\[2 + 2 + 2 + 2 + 3 + 3 + 3 + 3\\]
All I did was break up all the parentheses and group equal terms together.

Notice the repeated addition of twos and threes! This is multiplication! Let's write it as such!
...
Now why the hell did we do all this? Why can't we just go the simpler route and perform addition? Why do we break up multiplication and reintroduce it again?
...
If you take a look at the shape of this equation, you might notice something. Instead if multiplying the entire addition inside the parentheses by four,
...
I introducted distributivity as going from left to right. That is, breaking up multiplication and reordering the terms leads to this form.

But **equality is a two-way street**. There's nothing stopping me from going from right to left and writing:
...
The factored form of the expression is a lot easier to compute in your head. It involves "easy" numbers. Look:
...
What I did was grouping the first and last term together, the second and second to last together, and so on.
This is a simple reordering and regrouping.

Notice now, that each parentheses evaluates to 101. Really, I'm not joking, see for yourself. Replacing the parentheses with their value,
...
Well, if you look at the sum above, each parentheses in the reordering contains 2 terms. How many such groups of two can we make from 100 elements?
Obviously, the answer is 50. There are 50 such groups.

This means that 101 is multiplied by 50. And we have our answer:
\\[50 \cdot 101 = 5050\\]

This is the sum of number from 1 to 100.

# Takeaway

We knew a lot about addition and multiplication individually, but only now have we tried to combine them.
...
The last problem demonstrate how hard and tedious problems can be solved much quicker if we make good use
